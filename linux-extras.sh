...
# false even though a user is sitting there. Probing /dev/tty asks the terminal
# directly, which is what makes the advertised one-liner actually install these.
have_sudo() {
    sudo -n true 2>/dev/null && return 0
    [[ -e /dev/tty ]] && sudo -v </dev/tty 2>/dev/null && return 0
    return 1
}
...
if command -v gcloud >/dev/null 2>&1; then
    ...
    # keyring and blocks the installer. The keyring outlives the package, so
    # this fires on any re-run after a failed apt step, after `apt remove`, or
    # for anyone who already followed Google's docs by hand.
    if ensure_gnupg &&
